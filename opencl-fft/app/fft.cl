...
real2_t twiddle_kalpha(real2_t a, real_t kalpha)
{
	real_t cs, sn;
...
__kernel void fft_radix2(__global const real2_t * x, __global real2_t * y, int p,
					int pminus1, real_t minusPIoverp, int len)
{
	...
	//get what data set we are working on and increment pointers by len * d_set
	int d_set = get_global_id(1);
	i += d_set * len;
	j += d_set * len;
...
__kernel void fft_radix4(__global const float2 * x, __global float2 * y, int p, int pminus1,
					int twop, int threep, real_t minusPIover2p, real_t minusPIover2p_2x,
					real_t minusPIover2p_3x, int len)
{
	int t = get_global_size(0); // thread count
	int i = get_global_id(0);   // thread index
